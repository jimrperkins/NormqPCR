...
@
TO BE FIXED: We load efficiencies for the dataset and add them to the dataset.
<<ComputeNRQs3>>=
...
@
Now we can compute normalized relative quantities for the dataset where we 
consider two of the included features as reference/housekeeping genes.
<<ComputeNRQs4>>=
...
geometric averiging of multiple internal control genes.
...
\newblock qBase relative quantification framework and software for management and 
automated analysis of real-time quantitative PCR data.
\newblock Genome Biology 2007, 8:R19
